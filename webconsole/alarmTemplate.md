...
告警模板与USDP集群绑定，及可对该集群实施监控，当触发监控规则是，即及时向 [通知组](usdp_community/webconsole/alarmInform_group) 中的各个 [通知对象](usdp_community/webconsole/alarmInform_object) 发送告警信息。
...
- [模板监控指标管理](usdp_community/webconsole/alarmTemplate?id=_1-模板监控指标管理)
- [告警模板快速绑定集群并生效](usdp_community/webconsole/alarmTemplate?id=_2-告警模板快速绑定集群并生效)
- [新建告警模板](usdp_community/webconsole/alarmTemplate?id=_3-新建告警模板)
- [删除告警模板](usdp_community/webconsole/alarmTemplate?id=_4-删除告警模板)
...
!> USDP提供的 默认告警模板 不允许修改告警项。如需调整，可复制默认告警模板来调整具体告警指标，参见 [新建告警模板](usdp_community/webconsole/alarmTemplate?id=_3-新建告警模板) 。
...
关于自定义告警模板，参见 [新建告警模板](usdp_community/webconsole/alarmTemplate?id=_3-新建告警模板) 。
...
!> USDP预置的 “监控指标” 不允许用户自行删除，用户可选择将相应的 “监控指标” 设为不启用，参考 [给监控指标设置启停](usdp_community/webconsole/alarmTemplate?id=_14-给监控指标设置启停) 即可。
...
用户亦可在您自己的集群上来设置绑定哪些告警模板，相关操作请参考 [告警生效](usdp_community/webconsole/alarmTemplate_work?id=_方式2-集群告警设置) 章节。

?> 为告警模板绑定集群后，即代表此集群已使用该模板中的所有监控指标，对该集群中的相应服务实施监控，当集群一但发生相关告警规则设置的条件时，即会触发告警，USDP将会通过该模板中事先设置好的 [通知组](usdp_community/webconsole/alarmInform_group) 来发送告警详细信息。
